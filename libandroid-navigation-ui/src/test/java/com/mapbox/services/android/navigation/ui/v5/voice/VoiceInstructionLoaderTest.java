...
public class VoiceInstructionLoaderTest extends BaseTest {
  ...
  @Test
  public void checksRequestNotEnqueuedIfNoConnection() {
    Cache anyCache = mock(Cache.class);
    MapboxSpeech.Builder aSpeechBuilder = mock(MapboxSpeech.Builder.class);
    ConnectivityStatusProvider connectivityStatus = mock(ConnectivityStatusProvider.class);
    when(connectivityStatus.isConnected()).thenReturn(false);
    VoiceInstructionLoader theVoiceInstructionLoader = new VoiceInstructionLoader("any_access_token", anyCache,
      aSpeechBuilder, connectivityStatus);
    List<String> urlsToBeCached = buildUrlsToCache();

    theVoiceInstructionLoader.cacheInstructions(urlsToBeCached);

    verifyZeroInteractions(aSpeechBuilder);
  }
